...
class FlutterDriver extends driver_1.BaseDriver {
    ...
    async createSession(...args) {
        ...
        return session_1.createSession.bind(this)(sessionId, caps, ...JSON.parse(JSON.stringify(args))); // @ts-ignore
    }
    async deleteSession() {
        await Promise.all([
    ...
    async executeCommand(cmd, ...args) {
        if (cmd === `receiveAsyncResponse`) {
            ...
            throw new driver_1.errors.NoSuchDriverError(`Driver is not ready, cannot execute ${cmd}.`);
        }
    }
    getProxyAvoidList() {
